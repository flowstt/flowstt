            ...
            if self.is_speaking {
                ...
                if self.in_word_break {
                        ...
                        && self.word_break_sample_count <= self.max_word_break_samples 
                    {
                        ...
                        tracing::debug!("Word break detected (offset: {}ms, gap: {}ms)", offset_ms, gap_duration_ms);
                    }
                    self.in_word_break = false;
                    self.word_break_sample_count = 0;
                }
                ...
                if is_voiced {
                    ...
                    if self.voiced_onset_count >= self.voiced_config.onset_samples {
                        ...
                        tracing::debug!("Speech started (voiced mode, lookback: {}ms)", lookback_offset_ms);
                        return;
                    }
                }
                ...
                if is_whisper {
                    ...
                    if !self.is_speaking && self.whisper_onset_count >= self.whisper_config.onset_samples {
                        ...
                        tracing::debug!("Speech started (whisper mode, lookback: {}ms)", lookback_offset_ms);
                    }
                }
            }
        } else {
...
impl VisualizationProcessor {
    ...
    fn build_color_lut() -> Vec<[u8; 3]> {
        let stops = [
            ColorStop { position: 0.00, r: 10, g: 15, b: 26 },
            ColorStop { position: 0.15, r: 0, g: 50, b: 200 },
            ColorStop { position: 0.35, r: 0, g: 255, b: 150 },
            ColorStop { position: 0.60, r: 200, g: 255, b: 0 },
            ColorStop { position: 0.80, r: 255, g: 155, b: 0 },
            ColorStop { position: 1.00, r: 255, g: 0, b: 0 },
        ];
